 ...
 * Name: Huy Pham
 * Date: 02/15/2015
 * Solution description: Implementation of the binary search tree data structure.
 * 
 */
#include <stdlib.h>
#include <stdio.h>
...
/*
 ...
 Use the compare() function to compare values.
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while(current != 0)
	{ 
	    	if(compare(current->val, val) == 0)	//value found 
		{
			return 1;
		}
		else if(compare(current->val, val) < 0)	//current value is less
		{					//than search value
			current = current->right;	//go right
		}
		else					//current value is greater
		{					//than search value
