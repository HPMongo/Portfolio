 ...
 * to encrypt the content of a file using a one-time key pad.
 */
 ...
 * server for encryption. Result of the encryption will be displayed back
 * on the console.
 */
...
///			printf("Error reading from the socket -client\n");
		...
		else		// print encrypted message from server
		{
			printf("%s\n", buffer);
		}
	}
