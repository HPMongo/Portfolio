 ...
 * Name: Huy Pham
 * Date: 03/01/2015
 * Solution description: Dynamic Array implementation of various ADTs.
 *
 */
#include <assert.h>
#include <stdlib.h>
...
{
  /* FIXME */
	//printf("In smaller index heap - \n");
 	assert(heap != 0);
	assert(i < heap->size);
	assert(j < heap->size); 
	if(compare(getDynArr(heap,i), getDynArr(heap,j)) == -1) 	//data[i] < data[j]
		return i;
	else
		return j;
}
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	//printf("In addHeap - addingDynArr\n");	
	print_type(val);
	addDynArr(heap, val);
	while(pos != 0)
	{
	//	printf("In perculating logic - pos: %d\n", pos);
		parent = (pos - 1)/2;
	//	printf("parent: %d \n", parent);
...
{
   /* FIXME */
	//printf("In adjustHeap -");
	assert(max <= heap->size);
	int leftIdx = pos * 2 + 1;
	int rightIdx = pos * 2 + 2;
	int smallerIdx; 
	//printf("Max: %d Pos: %d Left: %d Right: %d\n", max, pos, leftIdx, rightIdx);

	if(rightIdx < max)	//Two children
	{
	//	printf("Two children -\n");
		smallerIdx = _smallerIndexHeap(heap, leftIdx, rightIdx);
	//	printf("Smaller idx is: %d\n", smallerIdx);
		if(compare(getDynArr(heap,smallerIdx),getDynArr(heap, pos)) == -1)
		{
	//		printf("Child < parent - swapping -\n");
			swapDynArr(heap, smallerIdx, pos);
	//		printf("Process with adjusting heap - \n");
			_adjustHeap(heap, max, smallerIdx);
		} 	
	}
	else if(leftIdx < max)	//One child
	{
	//	printf("One child - \n");
		if(compare(getDynArr(heap, leftIdx), getDynArr(heap, pos)) == -1)
		{
	//		printf("Left child < parent - swapping -\n");
